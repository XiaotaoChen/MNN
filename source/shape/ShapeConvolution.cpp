...
class ConvolutionCustomSizeComputer : public SizeComputer {
public:
    virtual bool onComputeSize(const MNN::Op* op, const std::vector<Tensor*>& inputs,
                               const std::vector<Tensor*>& outputs) const override {
        MNN_ASSERT(inputs.size() >= 1);
        ...
        auto layer        = op->main_as_ConvolutionCustom()->common_param();
        int kernel_width  = layer->dilateX() * (layer->kernelX() - 1) + 1;
        int kernel_height = layer->dilateY() * (layer->kernelY() - 1) + 1;
...
REGISTER_SHAPE(ConvolutionCustomSizeComputer, OpType_ConvolutionCustom);
